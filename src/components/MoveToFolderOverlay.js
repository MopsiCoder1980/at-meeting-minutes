...
export default function MoveToFolderOverlay({ minuteId, minuteTitle, folders, currentFolderId, onClose }) {
  ...
  return (
    <div className={styles.backdrop} onMouseDown={onClose}>
      <div className={styles.modal} onMouseDown={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Verschieben nach">
        <div className={styles.header}>
          <h2 className={styles.title}>Verschieben nach</h2>
        ...
        {folders.length === 0 && (
          <p className={styles.empty}>Noch keine Ordner angelegt.</p>
        )}
      </div>
    </div>
  )
