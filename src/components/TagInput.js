          ...
          <div className={styles.wrapper} onClick={() => inputRef.current?.focus()}>
               {tags.map(tag => (
                    <span key={tag} className={styles.pill}>
                         {tag}
                         <button type="button" className={styles.remove} onClick={e => { e.stopPropagation(); removeTag(tag) }} aria-label={`"${tag}" ${t('remove')}`}>×</button>
                    </span>
               ))}
               <input
                    ref={inputRef}
                    ...
                    placeholder={tags.length === 0 ? t('inputPlaceholder') : ''}
               />
